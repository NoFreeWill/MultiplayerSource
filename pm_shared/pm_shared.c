...
#define PLAYER_FATAL_FALL_SPEED		5000//orig 1024 approx 60 feet, %changed to 5000
#define PLAYER_MAX_SAFE_FALL_SPEED	2500//orig 580 approx 20 feet, %changed to 2500
#define DAMAGE_FOR_FALL_SPEED		(float) 100 / ( PLAYER_FATAL_FALL_SPEED - PLAYER_MAX_SAFE_FALL_SPEED )// damage per unit per second.
#define PLAYER_MIN_BOUNCE_SPEED		200
#define PLAYER_FALL_PUNCH_THRESHHOLD (float)350 // won't punch player's screen/make scrape noise unless player falling at least this fast.

#define PLAYER_LONGJUMP_SPEED 800 //orig 350 how fast we longjump, %changed to 800
...
void PM_UpdateStepSound( void )
{
	...
	{
		velwalk = 500; //%changed from 120
		velrun = 1000; //%changed from 210
		flduck = 0;
	}
...
void PM_AirAccelerate (vec3_t wishdir, float wishspeed, float accel)
{
	...
	if (wishspd > 30) //%changed to 3000 from 30
		wishspd = 30; //ditto
...
// see if standing on something solid	
	...
	else
	{
		...
		// If we hit a steep plane, we are not on ground, %changed from 0.7 to 0.3
		if ( tr.plane.normal[2] < 0.3)
...
// Only allow bunny jumping up to %changed from 1.7 to 1.0x server / player maxspeed setting
...
void PM_Jump (void)
{
	...
	if ( ( pmove->bInDuck ) || ( pmove->flags & FL_DUCKING ) )
	{
			...
			Length( pmove->velocity ) > 50 )
		{
			...
			/*for (i =0; i < 2; i++)
			{
				pmove->velocity[i] = curspeed; //pmove->forward[i] * PLAYER_LONGJUMP_SPEED * 1.6;
			}
			*/
			pmove->velocity[2] = curspeed; //%changed 2 at start to 500
		}
		else
		{
			pmove->velocity[2] = curspeed; //%changed 45 at end to 90
		}
	}
	else
	{
		pmove->velocity[2] = curspeed;//%changed 800 to 6000 used to be sqrt(15 * 800 * 45.0)
	}
